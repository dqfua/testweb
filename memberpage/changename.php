...
if ( $nsubmit )
{
	...
	//$goname = CGlobal::DecodeName( @CNeoInject::sec_Thai( CGlobal::__POST("chaname") ) );
	...
	//replace security string to empty
	$goname = str_replace( $sql_inject_2 , "" , $goname );
...
เปลี่ยนชื่อตัวละครเป็น : <input name="changename" id="changename" type="text" value="<?php echo $pCha->GetChaName(); ?>"  <?php if ( $NewPoint < 0 ) echo "disabled"; ?> > 
 <a href="#checkname" onClick="pp_checkname( '../popup/checkname.php',$('#changename').val(),<?php echo $_CONFIG["SERVERMAN"]["SERVER_MEMNUM"]; ?> )"><b> เช็คชื่อ </b></a></div></td>
      </tr>
      <tr>
        <td width="34%">ไอดี</td>
        <td width="66%"><label>
      ...
      <tr>
        <td>&nbsp;</td>
        <td><a href="#checkname" onClick="<?php if ( $NewPoint >= 0 ) { ?>pp_changename( '../popup/checkname.php',$('#changename').val(),<?php echo $_CONFIG["SERVERMAN"]["SERVER_MEMNUM"]; ?> ) <?php  } else { echo "alert('พ้อยของคุณไม่เพียงพอ!!');"; }?>">
