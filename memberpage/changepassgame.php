...
if ( $nsubmit )
{
	$passdel = CInput::GetInstance()->GetValueString( "passdel" , IN_POST );
	...
	if ( $_CONFIG["PASSWORD_MD5"][$cWeb->GetServerType()] )
	{
		...
		CGlobal::SetPassMD5( $passdel , true );
	}
	//printf( "%s<br>", $oldpass );
	...
	$backup_pass = $pass;
	if ( strcmp( $cUser->GetUserPass2() , $passdel ) != 0 )
	{
		die("รหัสลบตัวละครไม่ถูกต้อง<br>");
	}
	if ( strcmp( $cUser->GetUserPass() , $oldpass ) != 0 )
	{
	...
	$cUser->UpdatePassGame( $pass );
	
	CNeoLog::LogUpdateGamePassword( $_CONFIG["SERVERMAN"]["SERVER_MEMNUM"] , $UserNum , $backup_pass , CGlobal::getIP() );
	
	echo "เปลี่ยนรหัสเข้าเกมส์สำเร็จ<br>";
...
<table width="600" border="0" align="center" cellpadding="5" cellspacing="5" bgcolor="#000000">
  <tr>
    <td width="100%" align="left" valign="top"><table width="100%" border="0" cellpadding="3" cellspacing="3">
      <tr>
        <td colspan="2" align="center"><strong>เปลี่ยนรหัสเข้าเกม</strong></td>
        </tr>
      <tr>
        <td width="34%">ไอดี</td>
        <td width="66%"><label>
        ...
        <tr>
        <td>รหัสลบตัวละคร</td>
        <td><input type="password" name="passdel" id="passdel" /></td>
        </tr>
      <tr>
        <td>รหัสผ่านเก่า</td>
      ...
      <tr>
        <td>&nbsp;</td>
        <td>
        <a href="#c" onclick="changepassgame();">
