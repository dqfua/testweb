...
$arrItemOption = array( "NULL" , "DAMAGE(%)" , "DEFENSE(%)" , "HITRATE(+%)" , "AVOIDRATE(+%)" , "HP" , "MP" , "SP" , "HP_INC" , "MP_INC" , "SP_INC" , "HMS_INC" , "GRIND_DAMAGE"
                         , "GRIND_DEFENSE" , "ATTACK_RANDOM" , "DIS_SP" , "RESIST" , "ความเร็วในการเคลื่อนที่" , "โจมตี" , "ป้องกัน" , "อัตราโจมตี" , "อัตราหลับหลีก" , "พลัง" , "ความแกร่ง" , "ความฉลอด"
                         , "ความว่องไว" , "ความอดทน" , "ต่อสู้ระยะประชิด" , "การยิง" , "พลังงาน" , "ฟื้นฟู HP" , "ฟื้นฟู MP" , "ฟื้นฟู SP" , "ฟื้นฟู CP" );

//0x0112
define("_0112_CHAINVEN_ITEM_SLOT",160);
define("_0112_CHAINVEN_VERSION",0x0112);

//0x0113
define("_0113_CHAINVEN_ITEM_SLOT",176);
define("_0113_CHAINVEN_VERSION",0x0113);


define("CHAINVEN_HEAD",24);

define("CHAINVEN_MAXITEM",48);
define("INVEN_X",5);
define("INVEN_Y",8);
define("ITEM_ERROR",-1);

$PIECE_POS_TYPE = array( "หัว" , "เสื้อ" , "กางเกง" , "ถุงมือ" , "รองเท้า" , "อาวุธ L" , "อาวุธ L หลัง" , "สร้อยคอ" , "ต่างหู" , "แหวน R" , "แหวน L" , "อาวุธ R" , "อาวุธ R หลัง" , "ยานพาหนะ" , "Belt" , "Earring" , "Accessory" , "Extra 1" , "Extra 2"  );
...
class _CNeoChaInven
{
    ...
    public $Item_dwPetID = array();
    public $Item_dwVehicleID = array();
    public $Item_bVietnamGainItem = array();
    public $Item_DummyC1 = array();
    public $Item_DummyC2 = array();
    public $Item_DummyC3 = array();
    public $Item_DummyC4 = array();
    public $Item_DummyC5 = array();
    public $Item_DummyC6 = array();
    public $Item_DummyC7 = array();
    //suit change color
    public $Item_bSubmitColor = array();
    public $Item_dwItemColor1 = array();
    public $Item_dwItemColor2 = array();
    ...
    public function AddItem( &$cNeoSerialMemory , $pos = ITEM_ERROR )
    {
            if ( $this->ChaPutOnItemsMode == false )
            {
                switch( $this->Version )
                {
                        case _0112_CHAINVEN_VERSION:
                        {
                                self::LoadNode_0112( $this->ItemSlotNum , $cNeoSerialMemory );
                        }break;
                        case _0113_CHAINVEN_VERSION:
                        {

                                self::LoadNode_0113( $this->ItemSlotNum , $cNeoSerialMemory );
                        }break;
                ...
                switch( $this->Version )
                {
                        ...
                        case _0113_CHAINVEN_VERSION:
                        {
                                ...
                                self::LoadNode_0113( $pos , $cNeoSerialMemory );
                        }break;
                        default: return false;
                }
    ...
    public function DeleteItem( $ItemID )
    {
            ...
            for( $i = $ItemID ; $i < $this->ItemSlotNum ; $i++  )
            {
                    ...
                    if ( $this->Version == _0112_CHAINVEN_VERSION )
                    {
                            self::LoadNode_0112( $i , $cNeoSerialMemory );
                    }else if ( $this->Version == _0113_CHAINVEN_VERSION )
                    {
                            self::LoadNode_0113( $i , $cNeoSerialMemory );
                    }
    ...
    public function UpdateBinaryItemSlot( $i )
    {
            ...
            //PetID
            $cNeoSerialMemory->WriteInt( $this->Item_dwPetID[$i] );
            //VehicleID
            $cNeoSerialMemory->WriteInt( $this->Item_dwVehicleID[$i] );
            //VietnamGainItem
            $cNeoSerialMemory->WriteByte( $this->Item_bVietnamGainItem[$i] );
            //?
            $cNeoSerialMemory->WriteByte( $this->Item_DummyC1[$i] );
            $cNeoSerialMemory->WriteByte( $this->Item_DummyC2[$i] );
            $cNeoSerialMemory->WriteByte( $this->Item_DummyC3[$i] );
            $cNeoSerialMemory->WriteByte( $this->Item_DummyC4[$i] );
            $cNeoSerialMemory->WriteByte( $this->Item_DummyC5[$i] );
            $cNeoSerialMemory->WriteByte( $this->Item_DummyC6[$i] );
            $cNeoSerialMemory->WriteByte( $this->Item_DummyC7[$i] );
            //suit change color system
            //$cNeoSerialMemory->WriteByte( $this->Item_bSubmitColor[$i] );
            $cNeoSerialMemory->WriteInt( $this->Item_dwItemColor1[$i] );
            $cNeoSerialMemory->WriteInt( $this->Item_dwItemColor2[$i] );
    ...
    public function LoadHead_0113( &$cNeoSerialMemory )
    {
            $this->ItemSlotNum = $cNeoSerialMemory->ReadInt();
    }
    ...
    {
            $cNeoSerialMemory->WriteInt( _0112_CHAINVEN_VERSION );
            $cNeoSerialMemory->WriteInt( 64 ); // ???
            $cNeoSerialMemory->WriteInt( $this->ItemSlotNum );
    }
    public function SaveHead_0113( &$cNeoSerialMemory )
    {
            $cNeoSerialMemory->WriteInt( _0113_CHAINVEN_VERSION );
            $cNeoSerialMemory->WriteInt( 0x58 ); // ???
            $cNeoSerialMemory->WriteInt( $this->ItemSlotNum );
    }
    ...
    public function LoadNode_0112( $i , &$cNeoSerialMemory )
    {
            ...
            //PetID
            $this->Item_dwPetID[$i] =  $cNeoSerialMemory->ReadInt();
            //VehicleID
            $this->Item_dwVehicleID[$i] =  $cNeoSerialMemory->ReadInt();
            //VietnamGainItem
            $this->Item_bVietnamGainItem[$i] =  $cNeoSerialMemory->ReadByte();
            //?
            $this->Item_DummyC1[$i] = $cNeoSerialMemory->ReadByte();
            $this->Item_DummyC2[$i] = $cNeoSerialMemory->ReadByte();
            $this->Item_DummyC3[$i] = $cNeoSerialMemory->ReadByte();
            $this->Item_DummyC4[$i] = $cNeoSerialMemory->ReadByte();
            $this->Item_DummyC5[$i] = $cNeoSerialMemory->ReadByte();
            $this->Item_DummyC6[$i] = $cNeoSerialMemory->ReadByte();
            $this->Item_DummyC7[$i] = $cNeoSerialMemory->ReadByte();
    }
    public function LoadNode_0113( $i , &$cNeoSerialMemory )
    {
            if ( $this->ChaPutOnItemsMode == false )
            {
                //Item X , Y
            ...
            $this->Item_DummyC7[$i] = $cNeoSerialMemory->ReadByte();
            //
            //$this->Item_bSubmitColor[$i] = $cNeoSerialMemory->ReadByte();
            $this->Item_dwItemColor1[$i] = $cNeoSerialMemory->ReadInt();
            $this->Item_dwItemColor2[$i] = $cNeoSerialMemory->ReadInt();
    }
    ...
    public function Load_0112( &$cNeoSerialMemory )
    {
            ...
            {
                    $this->Binary_ItemSlot[$i] = $cNeoSerialMemory->ReadBuffer( _0112_CHAINVEN_ITEM_SLOT );
    ...
    public function Load_0113( &$cNeoSerialMemory )
    {
            //slot size 128 byte
            $cNeoSerialMemory->SetSeekNow();
            for( $i = 0 ; $i < $this->ItemSlotNum ; $i++ )
            {
                    $this->Binary_ItemSlot[$i] = $cNeoSerialMemory->ReadBuffer( _0113_CHAINVEN_ITEM_SLOT );
                    //echo $this->Binary_ItemSlot[$i]."<br>";
            }
            $cNeoSerialMemory->SetToDefault();
            for( $i = 0 ; $i < $this->ItemSlotNum ; $i++ )
    ...
    public function Save_0113( &$cNeoSerialMemory )
    {
            //24 byte
            self::SaveHead_0113( $cNeoSerialMemory );
            for( $i = 0 ; $i < $this->ItemSlotNum ; $i++ )
            {
                    //128 byte item slot
                    //self::UpdateBinaryItemSlot( $i );
    ...
    public function LoadChaPutOnItems( $Binary )
    {
            ...
            switch( $this->Version )
            {
                    ...
                    case _0113_CHAINVEN_VERSION:
                    {
                            ...
                            self::LoadHead_0113( $cNeoSerialMemory2 );
                            self::Load_0113( $cNeoSerialMemory );
                    }break;
                    default:
                    {
                            self::ClearBuffer();
    ...
    public function SaveChaPutOnItems()
    {
            $cNeoSerialMemory = new CNeoSerialMemory;
            $cNeoSerialMemory->OpenMemory(  );
            self::Save_0113( $cNeoSerialMemory );
